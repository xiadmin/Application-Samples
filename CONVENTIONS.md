...
Use the current API/language-first layout under `samples/`:

```text
samples/
  xiapi/                    # C samples using xiAPI
...
- The canonical sample root is lowercase `samples/`.
...
- The root-style CMake entry lives at `cmake/CMakeLists.txt`, not at the repository root.
- `scripts/build.py` is the canonical full-repo build helper and should be able to call or otherwise reuse `cmake/CMakeLists.txt`.
