...
#define ABORT_PROGRAM  std::cout << "Aborting program due to errors!" << std::endl; exit(-1); //note that these are two instructions
                                                                                    //use with curly braces

#define CHECK_XI_RET_EXCEPT(str) if (ret != XI_OK) throw str;
#define CHECK_XI_RET(str) if (ret!=XI_OK) {std::cout<<"XIAPI error at: " << str << std::endl; ABORT_PROGRAM }
#define CHECK_CUDA_RET(str) if (ret!=cudaSuccess) {std::cout<<"CUDA error at: " << str << std::endl; ABORT_PROGRAM }
#define CHECK_NPP_RET(str) if (ret!=NPP_NO_ERROR) {std::cout<<"NPP error at: " << str << std::endl; ABORT_PROGRAM }
...
int OpenCamera(HANDLE & xiH, int & cfa_type, int & transport_format)
{
    ...
    try
    {
        ...
        */
        ...
        if (packing != XI_ON)
        {
            std::cout << "Packing was not set on automatically. Setting on manually." << std::endl;
        ...
        std::cout << "TRANSPORT FORMAT: " << std::hex << transport_format << std::endl;
    ...
    {
        std::cerr << "Error in opening camera: ";
...
int main()
{
    ...
    //Display parameters
    std::cout << "Image height: " << std::dec << height << std::endl;
    std::cout << "Image width: " << std::dec << width << std::endl;
    std::cout << "Buffer size: " << std::dec << raw_buffer_length << std::endl;
    ...
    for (int it=0; it<NUM_CYCLES; it++)
    {
        ...
        std::cout << "Processing frame " << it << std::endl;
        ...
        std::cout << "NPPI demosaicing!" << std::endl;
        ...
        if (ret!= cudaSuccess)
        {
            std::cerr << "Unknown failure during debayering!" << std::endl;
        }
        ...
        */
        ...
        std::cout << "Saving to .tif!" << std::endl;
    ...
    //Print statistics
    std::cout << "\nStatistics:" << std::endl;
    std::cout << "Average depacking time: " << (depacking_time/NUM_CYCLES)/1000.0 << " ms" << std::endl;
    std::cout << "Average demosaicing time: " << (demosaicing_time/NUM_CYCLES)/1000.0 << " ms" << std::endl;
    std::cout << "Average saving time: " << (saving_time/NUM_CYCLES)/1000.0 << " ms" << std::endl;
    std::cout << "Average Rendering time: " << (rendering_time/NUM_CYCLES)/1000.0 << " ms" << std::endl;
    std::cout << "\n" << std::endl;
    ...
    if (ret != cudaSuccess)
    {
        std::cerr << "Could not deallocate cap_image.bp variable! Clean memory leaks manually." << std::endl;
    }

    ret = cudaFree(depacked_buffer);
    if (ret != cudaSuccess)
    {
        std::cerr << "Could not deallocate depacked_buffer variable! Clean memory leaks manually." << std::endl;
    }
